...
bool BaseShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	result = D3DX11CompileFromFile(vsFilename, NULL, NULL, "BaseVertexShader", VS_VERSION.c_str(), D3D10_SHADER_ENABLE_STRICTNESS, 0, NULL, 
	...
	result = D3DX11CompileFromFile(psFilename, NULL, NULL, "BasePixelShader", PS_VERSION.c_str(), D3D10_SHADER_ENABLE_STRICTNESS, 0, NULL, 
	...
	result = device->CreateVertexShader(vertexShaderBuffer->GetBufferPointer(), vertexShaderBuffer->GetBufferSize(), NULL, &m_vertex_shader);
	if(FAILED(result))
	{
		return false;
	}
	...
	result = device->CreatePixelShader(pixelShaderBuffer->GetBufferPointer(), pixelShaderBuffer->GetBufferSize(), NULL, &m_pixel_shader);
	if(FAILED(result))
	{
		return false;
	}
					   ...
					   D3DXVECTOR4 diffuseColor)
{
	...
	result = deviceContext->Map(m_matrix_buffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
	if(FAILED(result))
	{
		return false;
	}
	...
	deviceContext->VSSetConstantBuffers(bufferNumber, 1, &m_matrix_buffer);
